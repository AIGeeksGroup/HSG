...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, predictions, supervision_matrix_masked, mask):
        ...
        if valid_entries > 0:
            loss = -torch.sum(losses) / valid_entries
        else:
            # Return zero loss that requires grad (use zeros_like to preserve gradient flow)
            # Use predictions.sum() * 0 to ensure gradient flow is preserved
...
class EntailmentLoss(nn.Module):
    """
    Hyperbolic entailment loss, completely following MERU's entailment loss implementation.
    
    This loss enforces that one set of features (e.g., text) should entail 
    another set of features (e.g., image) in hyperbolic space. The loss is 
    computed as the clamped difference between the angle and the aperture.
    
    Reference: MERU paper (https://arxiv.org/abs/2304.09172)
    """
    def __init__(self, aperture_threshold=1.0, curv_init=1.0, learn_curv=True):
        """
        Args:
            aperture_threshold: Multiplier for the aperture. Default is 1.0.
                               Can be set to different values for inter-modal (e.g., 0.7)
                               vs intra-modal (e.g., 1.2) entailment.
        ...
        """
        super(EntailmentLoss, self).__init__()
    ...
    def forward(self, feat1, feat2):
        ...
        Args:
            feat1: First set of features in hyperbolic space, shape (N, D)
                   Typically the "entailing" features (e.g., text).
            feat2: Second set of features in hyperbolic space, shape (N, D)
                   Typically the "entailed" features (e.g., image).
        
        Returns:
            loss: Scalar loss value.
        """
        # Clamp curvature to prevent training instability (similar to MERU)
        self.curv.data = torch.clamp(self.curv.data, **self._curv_minmax)
        _curv = self.curv.exp()
        
        # Compute all necessary loss components. We enclose the entire block with
        # autocast to force a higher floating point precision (similar to MERU).
        with torch.autocast(device_type=feat1.device.type, dtype=torch.float32):
            # Hyperbolic entailment loss: feat1 should entail feat2.
            _angle = L.oxy_angle(feat1, feat2, _curv)
            _aperture = L.half_aperture(feat1, _curv)

            entailment_loss = torch.clamp(_angle - self.aperture_threshold * _aperture, min=0).mean()
