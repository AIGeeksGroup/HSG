...
class TopoMapperHandler(object):
    ...
    def __init__(self, config, video_data_dir, video_id, curv=None):
        self.config = config
        ...
        # Get curvature for hyperbolic space mapping
        # If curv is provided (trained curvature from DecoderAssociator), use it; otherwise use curv_init from config
        if curv is not None:
            # curv should be the actual curvature value (already exp'd if from DecoderAssociator.curv.exp())
            if isinstance(curv, torch.Tensor):
                self.curv = curv.detach().cpu() if curv.requires_grad else curv.cpu()
            else:
                self.curv = torch.tensor(float(curv))
        else:
            # Fallback to curv_init from config
            associator_config = config.get('associator', {})
            curv_init = associator_config.get('curv_init', 100.0)
            self.curv = torch.tensor(curv_init)
    ...
    def object_update(self, image_id, detections, object_embeddings):
        ...
        # Map embeddings from hyperbolic space to Euclidean space before computing cosine similarity
        object_embeddings_euclidean = L.log_map0(object_embeddings, self.curv)
        object_feature_bank_euclidean = L.log_map0(self.object_feature_bank, self.curv)
        
        # cosine similarity 
        obj_sim = torch.cosine_similarity(object_embeddings_euclidean.unsqueeze(1), object_feature_bank_euclidean.unsqueeze(0), dim=-1)  # K x M
    ...
    def get_pp(self,):
        ...
        # Map embeddings from hyperbolic space to Euclidean space before computing cosine similarity
        place_embeddings_euclidean = L.log_map0(place_embeddings, self.curv)
        # calculate the pairwise cosine similarity matrix
        self.pp_adj_sim = torch.cosine_similarity(place_embeddings_euclidean.unsqueeze(1), place_embeddings_euclidean.unsqueeze(0), dim=-1)
...
class TopoMapperv2(object):
    ...
    def get_pp(self,):
        ...
        place_embeddings = torch.stack(place_embeddings, dim=0)
        # Map embeddings from hyperbolic space to Euclidean space before computing cosine similarity
        print("final_trained_curv", self.curv)
        place_embeddings_euclidean = L.log_map0(place_embeddings, self.curv)
        # calculate the pairwise cosine similarity matrix
        self.pp_adj_sim = torch.cosine_similarity(place_embeddings_euclidean.unsqueeze(1), place_embeddings_euclidean.unsqueeze(0), dim=-1)
        # threshold the matrix
