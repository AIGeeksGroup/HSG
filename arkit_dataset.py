    ...
    def __init__(self, video_data_dir, video_ids, configs, transforms, batch_size, split="train"):
        ...
        for vid in video_ids:
            try:
                ...
                # Skip videos that don't have the required annotation file
                print(f"Warning: Skipping video {vid} - {e}")
                continue

        # Filter video_ids to only include successfully loaded videos
        self.video_ids = [dt.video_id for dt in self.datasets]
        self.total_batch_size = batch_size
        if len(self.datasets) > 0:
            self.bs_per_video = batch_size // len(self.datasets)
        else:
            self.bs_per_video = batch_size
        ...
        if len(self.datasets) > 0:
            ...
            self.lens_per_video = torch.tensor([])
            self.min_len = 0
            self.max_len = 0
